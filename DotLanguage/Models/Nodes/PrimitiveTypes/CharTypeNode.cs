...
namespace DotLanguage.Models.Nodes.PrimitiveTypes
{
    public class CharTypeNode : Node
    {
        ...
        private static readonly Regex firstRegex = new Regex(@"^[S]*");
        private static readonly Regex validateRegex = new Regex(@"^Shar$");
        private static readonly Regex buildRegex = new Regex(@"^Sh*a*r*(?<=ch*a*r*)$");
